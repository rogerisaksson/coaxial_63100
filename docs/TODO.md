...
| | Value |
|---|---|
| `run_tests.ps1 -All` | 2431 checks, 25 suites |
...
| The attitude view at its asked rate, still at rest, with the parts outlined: the period frame to frame (10 -> 20 fps at --hz 20, measured), a 0.35-degree deadband under the display's own resolution so a resting board redraws bit-identical, and the parts outlined as a subtle braille wireframe from the export's EXACT creases - the slab's top measured from the mesh, parts a millimetre over it and the slab's own rim and bore, footprints included, loops past two cells wide (95 at the view's zoom, an 0805 the smallest), each loop's edges merged along their chains (0.15 mm chord, 778 edges from 1,458) and every dot on the matrix's own lines so nothing slides with the sampling (blink events in a tumble 6.0 to 2.7 a frame, measured), edges under half a cell and crease-dense loops skipped, hidden lines removed with a 0.6 mm grace so a top-side part never shows through the 1.6 mm slab from below, lone dots dropped - 620 cells at 150x44, each line +82 luma over the cell it sits on (measured; +51 read as "barely noticeable"), glinting to the ladder's top in the lamp's pool; the horizon and the perspective fan in the same braille, every line meeting the horizon and hazed toward it (aerial perspective, 40 % of the line's grey left at the horizon, overlapping lines not adding up); the board's own X, Y, Z as a braille triad in the lower left, turning with it, in the motif's three colours (X sodium, Y neon, Z ash), an axis toward the camera brighter, its letters following the tips a cell at a time (0 reversals in a 2-degree tumble, from 12), sized a sixth of the frame's rows; three frames voting per cell so nothing exists for a single frame (blink events 16.2 to 1.1 a frame over every cell, one frame of latency); and a POINT key light on the tone, Lambert on the screen-space normal, so a resting board carries a gradient and a part's wall falls into shade (luma band 42-163 at rest, 45 wide before) | FINDINGS, *The attitude view drew ten frames a second*; `coaxial/wireframe.py`, OUTLINE_DEG |
