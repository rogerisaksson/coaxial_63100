...
| (nothing said the swap exists) | "Jag kan inte byta till simulerad hårdvara. Jag är konfigurerad för att interagera med den fysiska kretskortet" - true about itself, and a dead end: the answer does not say the swap exists | one SYSTEM line naming `/board` and `/model`. It stopped the refusal and did not buy the right answer - re-measured twice, the same question reached for `link_diagnose` and then `analog_read`, and wrote nothing. Settled by taking it off the model entirely: `debug.board_switch` reads "byt till debugproben" as the order it is and swaps the board itself, no turn spent - the same rule as `language.bare_switch`. `link_diagnose` on a portless session also names the stand-in and says `/board auto`, for the phrasings the parser abstains on |
