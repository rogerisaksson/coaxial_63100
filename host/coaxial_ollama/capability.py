"""What this host can run, and which local model to run on it."""
import argparse
import json
import os
import platform
...
def _cpu_busy():
    """How much of the host is already spoken for, as a percentage."""
    if platform.system() != 'Windows':
        return _posix_busy()
    try:
        out = subprocess.check_output(
...
def probe(host='http://localhost:11434'):
    """Measure this host: what it has, and what is left of it."""
    ...
    return Host(cores=cores, threads=threads, ram_gb=ram,
...
# One host's desktop is not another's.
...
def choose(host, prefer='speed', reserve_gb=None, catalogue=None):
    ...
    if not affordable:
        ...
        return Choice(smallest['tag'], {'num_gpu': 0},
                      ...
                      % (ram, smallest['tag']),
                      warnings=['this host is under-specified for a local '
    ...
    return _hybrid(step, budget, host, vram, reserve_gb, [])


def _fitting(fits, catalogue, host, vram, reserve_gb, budget, prefer):
    ...
    why = ('%.0f GB card with %.1f GB already on it, %.1f GB held back for '
           ...
           'entirely on the GPU'
           % (vram, host.vram_used_gb, reserve_gb, budget, best['tag']))
    choice = Choice(best['tag'], {}, why, entry=best)
    if prefer != 'capability':
        return choice
    bigger = [e for e in catalogue
              if e['gb'] > best['gb'] and e['ram_gb'] <= host.ram_gb]
    if not bigger:
        choice.warnings.append(
            'nothing larger fits this host either way; speed and '
...
def _hybrid(entry, budget, host, vram, reserve_gb, warnings):
    ...
    why = (('%.0f GB card leaves %.1f GB after the reserve, which holds no '
           ...
           ('%s does not fit %.1f GB whole, so %d of its %d layers go on the '
            ...
            % (entry['tag'], budget, layers, entry['layers'], host.cores)))
    if cpu_only and host.cores < FEW_CORES:
        warnings.append('%d cores for a CPU-only model is going to be slow '
                        'enough to notice on every question' % host.cores)
    if not cpu_only:
        warnings.append('a split model measured about five times slower per '
                        'token than one wholly on the GPU')
    # Every tok/s figure in this file was measured on an idle host, and the
    # part of this choice that runs on the CPU is the part a busy host slows
    # down.
    if host.cpu_busy is not None and host.cpu_busy >= 40:
        warnings.append('this host is %.0f%% busy right now, and the CPU '
...
def report(host=None, prefer='speed', reserve_gb=None):
    """The whole thing as text, for a human at a bench."""
    host = host or probe()
    choice = choose(host, prefer=prefer, reserve_gb=reserve_gb)
    lines = ['host: ' + host.line(),
             '         (%s)' % ', '.join('%s via %s' % (k, v)
                                         for k, v in sorted(host.notes.items())),
             '',
             'model:   ' + choice.tag]
    if choice.options:
...
def pulled(host='http://localhost:11434'):
    """Tags already on this host, so a recommendation can prefer one."""
    try:
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        ...
        description='What this host can run, and which local model to run.')
    ...
    host = probe()
    if args.json:
        choice = choose(host, prefer=args.prefer, reserve_gb=args.reserve_gb)
        out = choice.as_dict()
        out['host'] = host.as_dict()
        print(json.dumps(out, indent=2, sort_keys=True))
        return 0
    print(report(host, prefer=args.prefer, reserve_gb=args.reserve_gb))
    return 0
