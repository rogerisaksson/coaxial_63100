...
Twenty suites, 1831 checks, sized from `host/tests/.counts.json` and so measured
rather than remembered: `test_structure.py` (398), `test_ollama_tools.py`
...
(14, every view drawn twice, no board), `test_ollama_language.py` (12),
...
* **Any 5 % step is a tier.** Suites join in order of seconds per check, so the
  ...
  this tree's 1831 checks are in those nine files.
...
hotter than the third, the full suite was started three times. None of the 1831
