...
| (nothing said the swap exists) | "Jag kan inte byta till simulerad hårdvara. Jag är konfigurerad för att interagera med den fysiska kretskortet" - true about itself, and a dead end: the answer does not say the swap exists | one SYSTEM line naming `/board` and `/model`. It stopped the refusal and did not buy the right answer - re-measured, the same question reached for `link_diagnose` instead, and there is no budget left to outbid that hint |
