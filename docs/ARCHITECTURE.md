...
board/       this hardware behind comms/inc/board.h: board_adc, board_cal,
             ...
             board_daq, board_drive, board_imu, board_angle, board_boot
             (the image's header, the handover slot, the way back)
...
boot/        the bootloader: boot_core.c, the state machine - device 11
             over erase, program, read and say - C11, host-tested;
             boot_main.c, its hardware at register level, the only file
             here touching one; its own linker script, startup and
             CMakeLists - the second image, at sector 0. docs/BOOT.md is
             the design; the application sits behind it at 0x08020000
```
...
`Board_BootInit` (the unit id and the termination a bootloader handed
over, or nothing),
...
`cmd_time.c`, `cmd_thermal.c`, `cmd_power.c`, `cmd_drive.c`,
`cmd_boot.c` (device 11's `state` and `stay`; the rest is the
bootloader's and refused in words). Every
...
host/coaxial/graphics/  the board renderer: wireframe is the pipeline and
                        carries the map; solids, creases, stereotype,
                        shading, ground, lines, triad, steady, engine,
                        mesh, crew, raster and preload one concern each,
                        with the face art and the blue-noise mask beside
...
attached client every 3 s. The renderers - `coaxial/graphics/` (`engine`, `wireframe` and
the modules under it: `solids`, `creases`, `stereotype`, `shading`,
`ground`, `lines`, `triad`, `steady`, `mesh`, `crew`, `raster`, `preload`;
the map is wireframe's docstring), `ascii3d`, `orientation`,
...
unit, its fitment, and the host's own provenance as host text. Its
first inquiry is the PRELOAD: behind the page a child process at low
priority decimates the model six times, indexes the outline's exact
mesh and fits the parts' primitives, and keeps the lot as one pickle
under the user's local application data (`coaxial/graphics/preload.py` - never
beside the model), when the machine has a gigabyte of memory and a
hundred megabytes of disk to spare; the readout prints each step as
it lands, or the refusal. A view opening on that pickle has its
solids in a tenth of a second instead of four.
...
| test_modbus_core.py | 78 | the Modbus core through ctypes, clock injected, the oracle's prefix sweep |
...
| test_boot.py | 18 | the master's side of the bootloader against the stand-in: the sequence on one node, the image kept when it is the one held, the refusals in words, a bus of four through the prefix search and the master with one node unplaced, both implementations held to one interface |
| test_boot_core.py | 45 | the bootloader's state machine through the host gcc on a RAM flash: the chunk stream, loss and re-send, the seal and the first word written last, the master dying, wrong crc and wrong type, the debugger's way in, the faults |
| test_mcp.py | 50 | the MCP server |
| test_render.py | 120 | the 3D engine stage by stage against an analytic oracle, the scanline halftone and the drawn rim |
