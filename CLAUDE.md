...
Twenty-five suites, 2262 checks, sized from `host/tests/.counts.json` and so
measured rather than remembered: `test_structure.py` (497),
...
`test_daq_api.py` (67, the acquisition front door against the
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  764 of this tree's 2262 checks are in those nine files.
