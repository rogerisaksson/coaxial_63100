...
Twenty-seven suites, 3051 checks, sized from `host/tests/.counts.json` and so
measured rather than remembered: `test_structure.py` (632),
...
port, no board), `test_views.py` (206, every view and the front page drawn
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  773 of this tree's 3051 checks are in those nine files.
