...
/* The version query, and the one command whose layout is frozen. */
static cmd_status_t h_version(rd_t *in, wr_t *out)
{
  ...
  /* Appended, which is the one thing this record allows. */
...
static cmd_status_t h_adc_table(rd_t *in, wr_t *out)
{
  /* Optional start index, so the table is not bounded by one reply: a row is
     18 bytes plus its names against a 252 cap, and seven channels came to
     197 while nine came to 254. */
...
static cmd_status_t h_adc_scan(rd_t *in, wr_t *out)
{
  ...
  /* The NTC needs the reference, which the AFE powers. */
...
static cmd_status_t h_clock(rd_t *in, wr_t *out)
{
  ...
  /* Appended: what the converters run at after the prescaler. */
...
static cmd_status_t h_afe(rd_t *in, wr_t *out)
{
  ...
  /* Through the reference count, not the pin. */
...
/* What the board is made of, rather than what it is wired to. */
#define CHANNELS_SUBSYSTEMS 3U
/* What is fitted, rather than what it can do. */
...
/** Kinds 1 and 2, paged from an optional `first`. */
...
{
  /* The map, and only the map - no reading. */
  if (rd_left(in) < 1U)
  {
...
static const cmd_desc_t CMD_TABLE[] =
{
  ...
  /* Variable: an optional start index, so the table is not bounded by one
     reply. */
