...
Twenty-six suites, 2885 checks, counted in `host/tests/.counts.json`
by `counts.py`:
...
| test_simulated.py | 233 | the stand-in and the renderers, the desk's braille bars, and the thermal stand-in identifying its ground truth - UNCERTAIN, CONVERGING, STABLE in a box, again under a fan, the record a file between runs |
...
| test_views.py | 171 | every view and the front page drawn twice, the rotor observer's geometry, its gauges on one scale, the bead's wake, its DRIVE box, the shaft angle's round face, the box column every page scrolls, and the thermal observer's board as a halftone with its parts marked |
