...
def test_smart_selection(report):
    ...
    for percent, expect in ((25, {table.STRUCTURE, table.CORE,
                                  ...
                                  table.THERMAL, table.BOOT_CORE, table.CTRL_CORE,
...
def test_picker(r):
    ...
    pick_tests.clientmod.Ollama = explode
    ...
    try:
        ...
        pick_tests.clientmod.Ollama = real_client
...
def test_the_terminal_keeps_the_mouse(r):
    ...
    r.check('and with no terminal it never takes it',
            keys.grab(True) is False and not keys.holding())

    keys.mouse = True                   # no terminal to drive, the state only
    r.check('asked, it takes it', keys.grab(True) and keys.holding())
    ...
    r.check('the render view zooms on a key', view['zoom'] > 1.0,
            '%.3f' % view['zoom'])
    show_render.act_on(['-', '-'], view)
    r.check('and back out again', view['zoom'] < 1.0, '%.3f' % view['zoom'])
...
ROSTER = (
    ...
    (test_mouse, ('runner',)),
    (test_the_terminal_keeps_the_mouse, ('runner',)),
)
