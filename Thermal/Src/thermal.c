/**
  ******************************************************************************
  * @file    thermal.c
  * @brief   The observer: integrate the network, then correct it with the two
  *          sensors.
  *
...
void thermal_defaults(thermal_cfg_t *cfg)
{
  ...
  cfg->node[THERMAL_DRIVERS].to_board    = 15.2f;   /* bryggan, ur 4-1 */
                  ...
                  float ntc_c, float tsen_c, float dt_s)
{
  ...
  /* Anchor. TSEN reads close to the board node, so it corrects the board
     directly; the NTC then only has to explain what is left, which is the
     drivers' own rise. Without TSEN the NTC carries both and the split is a
     guess - hence `settled` staying false. */
  const float k = THERMAL_ANCHOR_HZ * dt_s;

  if (!isnan(tsen_c))
  {
    th->t[THERMAL_BOARD] += k * (tsen_c - th->t[THERMAL_BOARD]);

    if (!isnan(ntc_c))
    {
      /* What the NTC sees beyond the board is the drivers' share of their
         rise. Invert that to correct the drivers node. */
      const float seen = ntc_c - th->cfg.ntc_offset - th->t[THERMAL_BOARD];
      if (th->cfg.ntc_sees_drivers > 0.01f)
      {
        const float implied = th->t[THERMAL_BOARD]
                              + seen / th->cfg.ntc_sees_drivers;
        th->t[THERMAL_DRIVERS] += k * (implied - th->t[THERMAL_DRIVERS]);
      }
      th->settled = true;
    }
  ...
  {
    /* Degraded: no corner sensor, so the drivers' rise cannot be separated
    ...
    const float bulk = thermal_board_from_ntc(&th->cfg, ntc_c, rise);
