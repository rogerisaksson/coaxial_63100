<#
    What the ollama daemon has to be started with, and whether the one
    running
    now was.
  ...
  Whether the daemon now running actually has the tuning.
        Two sources, because neither covers both ways a daemon gets started:
          * `srv load_model: prompt cache is disabled` in server.log is what
            ...
            process started, everything in it belongs to this process and
            the
            newest such line is the answer.
            the ordinary one.
          * A daemon this script started writes to its own hidden console
          and
            never touches server.log, so the newest line there can belong to
            a daemon that exited hours ago.
            daemon on every single run - measured, twice.
            stamp below is the evidence: the environment was complete at
            that
...
#>
    ...
    if (Test-Path $log) {
        ...
        if ($written -gt $daemon.StartTime) {
            ...
            # The file is this daemon's but carries no load lines - it has not
            # been asked for a model yet, or the tray app is not capturing its
            # runner's output, which is what a tray started from anything but
            # Explorer does.
        }
    }
...
function Set-DaemonEnvironment {
  ...
  The tuning into this process and into the user's environment.
        ...
        app inherits at the next login - which is how the machine stays
        tuned
        without this script having to run first.
    ...
    foreach ($name in $script:DaemonTuning.Keys) {
        ...
        if ([Environment]::GetEnvironmentVariable($name, 'User') -ne $want) {
            try {
                ...
                # Silent by contract: this file prints nothing.
            }
        }
    }
    $marker = Get-TuningMarker
