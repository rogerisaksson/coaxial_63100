...
Twenty-six suites, 2536 checks, sized from `host/tests/.counts.json` and so
...
`test_thermal_core.py` (36, the SOA envelope as the C that will run -
...
(32, every view and the front page drawn twice, plus the rotor
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  766 of this tree's 2536 checks are in those nine files.
