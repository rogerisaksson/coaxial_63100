...
`coaxial_ollama/debug.py` `Chat` = `budget` (the prompt inside the
window), `turn` (one question, its rounds and calls) and `commands` (the /
lines); the fixed words are `words.py`.
