...
boot/        the bootloader's state machine, boot_core.c - device 11 over
             erase, program, read and say - C11; docs/BOOT.md is the
             design, and the application sits behind it at 0x08020000
```
...
Twenty-eight suites, 3206 checks, counted in `host/tests/.counts.json`
by `counts.py`:
...
| test_render.py | 118 | the 3D engine stage by stage against an analytic oracle, the scanline halftone and the drawn rim |
...
| test_views.py | 221 | every view and the front page drawn twice, the front page's readout, the rotor observer's geometry, its gauges on one scale, the bead's wake, its DRIVE box, the shaft angle's round face, the box column every page scrolls, and the thermal observer's board as a halftone with its parts marked |
