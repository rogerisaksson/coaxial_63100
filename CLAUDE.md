...
Twenty-six suites, 2878 checks, sized from `host/tests/.counts.json` and so
measured rather than remembered: `test_structure.py` (611),
`test_ollama_tools.py` (218), `test_ollama_runner.py` (223),
`test_simulated.py` (229), `test_live_model.py` (212, needs ollama, `--live`),
...
`test_thermal_core.py` (129, the SOA envelope as the C that will run -
...
(171, every view and the front page drawn twice, plus the rotor
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  773 of this tree's 2878 checks are in those nine files.
