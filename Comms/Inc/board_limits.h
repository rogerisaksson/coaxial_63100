/**
  ******************************************************************************
  * @file    board_limits.h
  * @brief   Every fixed number this board's code depends on, and why it is
  *          that number.
  *
  * A limit in the widest sense: what a part will take, what a buffer must
  * hold, how often something may run, and the floors under all three. They
  * were one #define per file, which is where they belong until the day one of
  * them is wrong - and then there is no way to see that IMU_CARGO is smaller
  * than IMU_BUF without opening two files in two layers.
  *
  * WHAT IS NOT HERE. Anything the board can be TOLD lives in the calibration
  * record instead (invariant 7): the dead time, the thermal ceilings, every
  * scaling parameter. This file holds what a rebuild is needed to change,
  * and the floor a stored value is checked against. Op codes, register maps
  * and channel indices stay in the one file that dispatches on them.
  *
  * Every number here carries the measurement that chose it. Moving one
  * without its comment is how it becomes a number nobody dares change.
  ******************************************************************************
  */
