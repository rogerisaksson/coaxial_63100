...
Twenty suites, 1840 checks, sized from `host/tests/.counts.json` and so measured
rather than remembered: `test_structure.py` (404), `test_ollama_tools.py`
(218), `test_ollama_runner.py` (214), `test_simulated.py` (194),
...
(28), `test_ollama_bus.py` (28), `test_ollama_reply.py` (23), `test_broker.py`
(22, the shared session, no board), `test_views.py`
(16, every view and the front page drawn twice, no board), `test_ollama_language.py` (12),
...
* **Any 5 % step is a tier.** Suites join in order of seconds per check, so the
  ...
  this tree's 1840 checks are in those nine files.
...
hotter than the third, the full suite was started three times. None of the 1840
