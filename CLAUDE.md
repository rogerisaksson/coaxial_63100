...
Twenty-eight suites, 3157 checks, sized from `host/tests/.counts.json` and so
...
port, no board), `test_views.py` (211, every view and the front page drawn
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  778 of this tree's 3157 checks are in those nine files.
