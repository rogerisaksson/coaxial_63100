...
* **A Check That Reached the Bench:** `test_link_diagnose` stubbed `comports`, `connect` and `check_power` but not `port_state`, which opens a real port. It passed with the probe connected and failed with it out, where COM4 answered `busy` and the checklist stopped a step earlier than the check expected - a suite testing the bench rather than the code, against its own docstring. Stubbed, and the BUSY branch it was reaching by accident now has a check of its own.
...
* **link_diagnose Misreads an Unplugged Probe:** False, and both readings that suggested it were the bench being measured while something else used it. `--power` answering `ST-LINK error (DEV_CONNECT_ERR)` in 2.3 s was taken while `run_tests.ps1 -All` was driving the same probe; with the gate finished the same call reads `0.00V` in 15.2 s. `port_state` answering `busy` was simply true - a `dbg.py --repl` left running from an earlier smoke test held COM4, which is what that line says. The branch built on them was reverted. Measure the bench when nothing else is on it.
* **Splitting `Chat` Would Decouple It:** False. Proposed as three collaborating objects - turn, steering, budget. Measured: all three touch `client`, `history`, `io_log`, `language` and `last_channels`; steering owns one attribute alone and budget owns none. Three objects would need a shared state struct all three hold, which is `Chat` renamed. The file is the class - 1088 of `debug.py`'s 1544 lines - so moving the wording (112) and the module helpers (105) out still leaves 1327 and costs two modules plus a re-export shim. A mixin split moves lines between files without reducing coupling, on the repository's most-changed file. Left alone; no class ceiling was added to the structure suite either, because mixins would defeat it and a module ceiling would only demand the refactor this refutes.
