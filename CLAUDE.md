...
Twenty-six suites, 2856 checks, sized from `host/tests/.counts.json` and so
...
`test_thermal_core.py` (127, the SOA envelope as the C that will run -
...
(164, every view and the front page drawn twice, plus the rotor
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  773 of this tree's 2856 checks are in those nine files.
