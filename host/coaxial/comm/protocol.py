...
class ImuOp(IntEnum):
    """Device 0, the BNO08X on SPI2 - `coaxial.devices.imu`."""
    ID = 0
    READ = 1
    FEATURE = 2
    PROBE = 3
...
class AngleOp(IntEnum):
    """Device 1, the A1335 on SPI4 - `coaxial.devices.angle`."""
    READ = 0
    WRITE = 1
    LATEST = 2
    HOLD = 3
...
class LinkOp(IntEnum):
    """Device 2, the link's own loopback and port counters - `coaxial.devices.link`."""
    ECHO = 0
    STATS = 1


class CalOp(IntEnum):
    """Device 3, the calibration record - `coaxial.devices.calibration`."""
    GET = 0
    SET_PARAM = 1
    SET_CHANNEL = 2
    ZERO = 3
...
class GateOp(IntEnum):
    """Device 4, TIM1 and the STO chain - `coaxial.devices.gate_drivers`."""
    STATE = 0
    PWM = 1
    DUTY = 2
    SYNC = 3
...
class LogOp(IntEnum):
    """Device 5, the measurement ring - `coaxial.acquire.capture`."""
    STATE = 0
    ARM = 1
    TAKE = 2
...
class DaqOp(IntEnum):
    """Device 6, the acquisition task - `coaxial.acquire.daq`."""
    STATE = 0
    CONFIGURE = 1
    START = 2
    STOP = 3
...
class TimeOp(IntEnum):
    """Device 7, the cycle counter - `coaxial.acquire.clock`."""
    LATCH = 0
    READ = 1
...
class PowerOp(IntEnum):
    """Device 9, the rails' reference counts - `coaxial.devices.power`."""
    STATE = 0
    RELEASE_ALL = 1


class BootOp(IntEnum):
    """Device 11, the bootloader - `coaxial.devices.boot`; a running application serves
...
class DriveOp(IntEnum):
    """Device 10, the control law - `coaxial.devices.drive`."""
    STATE = 0
    MODE = 1
    SETPOINT = 2
    SETPOINTS = 3
