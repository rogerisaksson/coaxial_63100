...
function Format-Argument {
  ...
  One argument, safe to hand to Start-Process.
        -ArgumentList joins an array with spaces and quotes nothing, so a
        value
        with a space in it arrives as several arguments.
        `-NewWindow -Ask "read the NTC"` reached the new window as -Ask
        read,
        the, NTC - and `NTC` then bound to -Prefer, which rejected it
        against
        its ValidateSet.
#>
    param([string]$Text)
