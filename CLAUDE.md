...
Twenty-eight suites, 3206 checks, sized from `host/tests/.counts.json` and so
...
`test_ollama_board.py` (28), `test_ollama_bus.py` (28), `test_render.py` (118,
...
port, no board), `test_views.py` (221, every view and the front page drawn
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  778 of this tree's 3206 checks are in those nine files.
