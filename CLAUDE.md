...
Twenty-seven suites, 3085 checks, sized from `host/tests/.counts.json` and so
...
port, no board), `test_views.py` (207, every view and the front page drawn
...
* **Any 5 % step is a tier.** Suites join by seconds per check - measured:
  ...
  773 of this tree's 3085 checks are in those nine files.
